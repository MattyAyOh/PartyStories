...
@interface PSFindFriendsViewController ()
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.objects.count) {
        return [PSFindFriendsCell heightForCell];
    } else {
        return 44.0f;
    }
}
...
- (PFQuery *)queryForTable {
    ...
    NSArray *facebookFriends = [[PSCache sharedCache] facebookFriends];
        ...
        if (!error) {
            if (number == self.objects.count) {
                ...
                for (PFUser *user in self.objects) {
                    [[PSCache sharedCache] setFollowStatus:YES user:user];
                }
            } else if (number == 0) {
                self.followStatus = PAPFindFriendsFollowingNone;
                [self configureFollowAllButton];
                for (PFUser *user in self.objects) {
                    [[PSCache sharedCache] setFollowStatus:NO user:user];
                }
            } else {
                self.followStatus = PAPFindFriendsFollowingSome;
                [self configureFollowAllButton];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    PSFindFriendsCell *cell = [tableView dequeueReusableCellWithIdentifier:FriendCellIdentifier];
    if (cell == nil) {
        cell = [[PSFindFriendsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:FriendCellIdentifier];
    ...
    NSDictionary *attributes = [[PSCache sharedCache] attributesForUser:(PFUser *)object];
    
    if (attributes) {
        // set them now
        NSNumber *number = [[PSCache sharedCache] photoCountForUser:(PFUser *)object];
        ...
        @synchronized(self) {
            ...
            if (!outstandingCountQueryStatus) {
                ...
                [photoNumQuery countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
                    @synchronized(self) {
                        [[PSCache sharedCache] setPhotoCount:[NSNumber numberWithInt:number] user:(PFUser *)object];
                        [self.outstandingCountQueries removeObjectForKey:indexPath];
                    }
                    PSFindFriendsCell *actualCell = (PSFindFriendsCell*)[tableView cellForRowAtIndexPath:indexPath];
    ...
    if (self.followStatus == PAPFindFriendsFollowingSome) {
        if (attributes) {
            [cell.followButton setSelected:[[PSCache sharedCache] followStatusForUser:(PFUser *)object]];
        } else {
            @synchronized(self) {
                ...
                if (!outstandingQuery) {
                    ...
                    [isFollowingQuery countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
                        @synchronized(self) {
                            ...
                            [[PSCache sharedCache] setFollowStatus:(!error && number > 0) user:(PFUser *)object];
                        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForNextPageAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PSLoadMoreCell *cell = [tableView dequeueReusableCellWithIdentifier:NextPageCellIdentifier];
    
    if (cell == nil) {
        cell = [[PSLoadMoreCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NextPageCellIdentifier];
...
- (void)followAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        for (int r = 0; r < self.objects.count; r++) {
            ...
            PSFindFriendsCell *cell = (PSFindFriendsCell *)[self tableView:self.tableView cellForRowAtIndexPath:indexPath object:user];
        ...
        [PSUtility followUsersEventually:self.objects block:^(BOOL succeeded, NSError *error) {
...
- (void)unfollowAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [PSUtility unfollowUsersEventually:self.objects];
...
- (void)shouldToggleFollowFriendForCell:(PSFindFriendsCell*)cell {
    PFUser *cellUser = cell.user;
    if ([cell.followButton isSelected]) {
        // Unfollow
        cell.followButton.selected = NO;
        [PSUtility unfollowUserEventually:cellUser];
        [[NSNotificationCenter defaultCenter] postNotificationName:PAPUtilityUserFollowingChangedNotification object:nil];
    } else {
        // Follow
        cell.followButton.selected = YES;
        [PSUtility followUserEventually:cellUser block:^(BOOL succeeded, NSError *error) {
            if (!error) {
                [[NSNotificationCenter defaultCenter] postNotificationName:PAPUtilityUserFollowingChangedNotification object:nil];
            } else {
                cell.followButton.selected = NO;
