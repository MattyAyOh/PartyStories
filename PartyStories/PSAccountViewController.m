...
- (void)viewDidLoad {
    ...
    [queryPhotoCount countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
        if (!error) {
            ...
            [[PSCache sharedCache] setPhotoCount:[NSNumber numberWithInt:number] user:self.user];
        }
    }];
...
- (void)followButtonAction:(id)sender {
    ...
    [PSUtility followUserEventually:self.user block:^(BOOL succeeded, NSError *error) {
        if (error) {
            [self configureFollowButton];
        }
    }];
...
- (void)unfollowButtonAction:(id)sender {
    ...
    [PSUtility unfollowUserEventually:self.user];
}
...
- (void)configureFollowButton {
    ...
    [[PSCache sharedCache] setFollowStatus:NO user:self.user];
}
...
- (void)configureUnfollowButton {
    ...
    [[PSCache sharedCache] setFollowStatus:YES user:self.user];
}
