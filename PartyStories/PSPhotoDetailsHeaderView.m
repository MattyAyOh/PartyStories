...
@interface PSPhotoDetailsHeaderView ()
...
@implementation PSPhotoDetailsHeaderView
...
- (void)drawRect:(CGRect)rect {
    [super drawRect:rect];
    [PSUtility drawSideDropShadowForRect:self.nameHeaderView.frame inContext:UIGraphicsGetCurrentContext()];
    [PSUtility drawSideDropShadowForRect:self.photoImageView.frame inContext:UIGraphicsGetCurrentContext()];
    [PSUtility drawSideDropShadowForRect:self.likeBarView.frame inContext:UIGraphicsGetCurrentContext()];
}
...
- (void)setLikeUsers:(NSMutableArray *)anArray {
    ...
    for (PSProfileImageView *image in currentLikeAvatars) {
    ...
    for (i = 0; i < numOfPics; i++) {
        PSProfileImageView *profilePic = [[PSProfileImageView alloc] init];
...
- (void)reloadLikeBar {
    self.likeUsers = [[PSCache sharedCache] likersForPhoto:self.photo];
    [self setLikeButtonState:[[PSCache sharedCache] isPhotoLikedByCurrentUser:self.photo]];
...
- (void)createView {    
    ...
    [self.photographer fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        // Create avatar view
        PSProfileImageView *avatarImageView = [[PSProfileImageView alloc] initWithFrame:CGRectMake(avatarImageX, avatarImageY, avatarImageDim, avatarImageDim)];
...
- (void)didTapLikePhotoButtonAction:(UIButton *)button {
    ...
    if (liked) {
        [[PSCache sharedCache] incrementLikerCountForPhoto:self.photo];
        [newLikeUsersSet addObject:[PFUser currentUser]];
    } else {
        [[PSCache sharedCache] decrementLikerCountForPhoto:self.photo];
    }
    
    [[PSCache sharedCache] setPhotoIsLikedByCurrentUser:self.photo liked:liked];

    [self setLikeUsers:[newLikeUsersSet allObjects]];

    if (liked) {
        [PSUtility likePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
        ...
        [PSUtility unlikePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
                [button addTarget:self action:@selector(didTapLikePhotoButtonAction:) forControlEvents:UIControlEventTouchUpInside];
                [self setLikeUsers:originalLikeUsersArray];
