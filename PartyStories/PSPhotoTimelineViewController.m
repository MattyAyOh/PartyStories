...
#import "PSLoadMoreCell.h"

@interface PSPhotoTimelineViewController ()
@property (nonatomic, assign) BOOL shouldReloadOnAppear;
@property (nonatomic, strong) NSMutableSet *reusableSectionHeaderViews;
@property (nonatomic, strong) NSMutableDictionary *outstandingSectionHeaderQueries;
@end

@implementation PSPhotoTimelineViewController
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    PSPhotoHeaderView *headerView = [self dequeueReusableSectionHeaderView];
    
    if (!headerView) {
        headerView = [[PSPhotoHeaderView alloc] initWithFrame:CGRectMake( 0.0f, 0.0f, self.view.bounds.size.width, 44.0f) buttons:PSPhotoHeaderButtonsDefault];
    ...
    NSDictionary *attributesForPhoto = [[PSCache sharedCache] attributesForPhoto:photo];

    if (attributesForPhoto) {
        [headerView setLikeStatus:[[PSCache sharedCache] isPhotoLikedByCurrentUser:photo]];
        [headerView.likeButton setTitle:[[[PSCache sharedCache] likeCountForPhoto:photo] description] forState:UIControlStateNormal];
        [headerView.commentButton setTitle:[[[PSCache sharedCache] commentCountForPhoto:photo] description] forState:UIControlStateNormal];
        ...
        @synchronized(self) {
            ...
            if (!outstandingSectionHeaderQueryStatus) {
                PFQuery *query = [PSUtility queryForActivitiesOnPhoto:photo cachePolicy:kPFCachePolicyNetworkOnly];
                [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                    @synchronized(self) {
                        ...
                        [[PSCache sharedCache] setAttributesForPhoto:photo likers:likers commenters:commenters likedByCurrentUser:isLikedByCurrentUser];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForNextPageAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PSLoadMoreCell *cell = [tableView dequeueReusableCellWithIdentifier:LoadMoreCellIdentifier];
    if (!cell) {
        cell = [[PSLoadMoreCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:LoadMoreCellIdentifier];
...
- (PSPhotoHeaderView *)dequeueReusableSectionHeaderView {
    for (PSPhotoHeaderView *sectionHeaderView in self.reusableSectionHeaderViews) {
...
- (void)photoHeaderView:(PSPhotoHeaderView *)photoHeaderView didTapLikePhotoButton:(UIButton *)button photo:(PFObject *)photo {
    ...
    if (liked) {
        ...
        [[PSCache sharedCache] incrementLikerCountForPhoto:photo];
    } else {
        if ([likeCount intValue] > 0) {
            likeCount = [NSNumber numberWithInt:[likeCount intValue] - 1];
        }
        [[PSCache sharedCache] decrementLikerCountForPhoto:photo];
    }
    
    [[PSCache sharedCache] setPhotoIsLikedByCurrentUser:photo liked:liked];
    
    [button setTitle:[numberFormatter stringFromNumber:likeCount] forState:UIControlStateNormal];
    
    if (liked) {
        [PSUtility likePhotoInBackground:photo block:^(BOOL succeeded, NSError *error) {
            PSPhotoHeaderView *actualHeaderView = (PSPhotoHeaderView *)[self tableView:self.tableView viewForHeaderInSection:button.tag];
        ...
        [PSUtility unlikePhotoInBackground:photo block:^(BOOL succeeded, NSError *error) {
            PSPhotoHeaderView *actualHeaderView = (PSPhotoHeaderView *)[self tableView:self.tableView viewForHeaderInSection:button.tag];
            [actualHeaderView shouldEnableLikeButton:YES];
            [actualHeaderView setLikeStatus:!succeeded];
...
- (void)photoHeaderView:(PSPhotoHeaderView *)photoHeaderView didTapCommentOnPhotoButton:(UIButton *)button  photo:(PFObject *)photo {
    PSPhotoDetailsViewController *photoDetailsVC = [[PSPhotoDetailsViewController alloc] initWithPhoto:photo];
