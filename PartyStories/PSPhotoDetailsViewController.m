...
#import "PSPhotoDetailsFooterView.h"
...
@interface PSPhotoDetailsViewController ()
@property (nonatomic, strong) UITextField *commentTextField;
@property (nonatomic, strong) PSPhotoDetailsHeaderView *headerView;
@property (nonatomic, assign) BOOL likersQueryInProgress;
@end

static const CGFloat kPAPCellInsetWidth = 20.0f;

@implementation PSPhotoDetailsViewController
...
- (void)viewDidLoad {
    ...
    // Set table header
    self.headerView = [[PSPhotoDetailsHeaderView alloc] initWithFrame:[PSPhotoDetailsHeaderView rectForView] photo:self.photo];
    self.headerView.delegate = self;
    
    self.tableView.tableHeaderView = self.headerView;
    
    // Set table footer
    PSPhotoDetailsFooterView *footerView = [[PSPhotoDetailsFooterView alloc] initWithFrame:[PSPhotoDetailsFooterView rectForView]];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    BOOL hasCachedLikers = [[PSCache sharedCache] attributesForPhoto:self.photo] != nil;
    if (!hasCachedLikers) {
        [self loadLikers];
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.objects.count) { // A comment row
        ...
        if (object) {
            ...
            return [PSActivityCell heightForCellWithName:nameString contentString:commentString cellInsetWidth:kPAPCellInsetWidth];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    PSBaseTextCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell == nil) {
        cell = [[PSBaseTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForNextPageAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PSLoadMoreCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil) {
        cell = [[PSLoadMoreCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        cell.cellInsetWidth = kPAPCellInsetWidth;
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.photo objectForKey:kPAPPhotoUserKey]) {
        ...
        [[PSCache sharedCache] incrementCommentCountForPhoto:self.photo];
        ...
        [comment saveEventually:^(BOOL succeeded, NSError *error) {
            ...
            if (error && error.code == kPFErrorObjectNotFound) {
                [[PSCache sharedCache] decrementCommentCountForPhoto:self.photo];
...
- (void)cell:(PSBaseTextCell *)cellView didTapUserButton:(PFUser *)aUser {
...
-(void)photoDetailsHeaderView:(PSPhotoDetailsHeaderView *)headerView didTapUserButton:(UIButton *)button user:(PFUser *)user {
